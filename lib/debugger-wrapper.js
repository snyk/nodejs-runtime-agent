  ...
  session.on('Debugger.scriptParsed', (script) => {
    ...
    if (vulnMgmt.isVulnerableModulePath(moduleInfo)) {
      ...
      Object.keys(vulnerableMethods).forEach((methodName) => {
        ...
        setBreakpointsOnVulnerableMethods(script.params.url, moduleInfo, methodName, methodLocation);
      });
    }
  });
...
function setBreakpointsOnVulnerableMethods(moduleUrl, moduleInfo, methodName, methodLocation) {
