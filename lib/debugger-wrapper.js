...
function instrumentScript(scriptUrl) {
  ...
  Object.keys(functionsToInstrument).forEach((functionName) => {
    const {location, publicId} = functionsToInstrument[functionName];
    if (!(functionName in scriptUrlToInstrumentedFunctions[scriptUrl])) {
      setBreakpointOnFunction(scriptUrl, moduleInfo, functionName, location, publicId);
...
function setBreakpointOnFunction(scriptUrl, moduleInfo, functionName, functionLocation, publicId) {
  ...
  session.post('Debugger.setBreakpointByUrl', breakpointParameters, (error,response) => {
    ...
    breakpointsMap[response.breakpointId] = {functionName, moduleInfo, scriptUrl, functionLocation, publicId};
...
function resumeSnoozedBreakpoints() {
  suspendedBreakpointIds.forEach((breakpointId) => {
    ...
    setBreakpointOnFunction(
      bpData.scriptUrl,
      bpData.moduleInfo,
      bpData.functionName,
      bpData.functionLocation,
      bpData.publicId);
  });
