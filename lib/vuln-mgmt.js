...
function isVulnerableModulePath(moduleInfo) {
    ...
    var scriptPathMethods = vulnerabiltiesMetadata[name][scriptRelativePath]; 
    var foundVulnerableMethod = false
    scriptPathMethods.forEach(value => {
        foundVulnerableMethod = foundVulnerableMethod || semver.satisfies(version, value.semver);
    });
...
function getVulnerableMethodsLocations(moduleInfo, scriptPath) {
    ...
    scriptPathMethods.forEach(value => {
        if (semver.satisfies(version, value.semver)) {
            vulnerableFunctionNames = vulnerableFunctionNames.concat(value.name)
        }
    }); 
